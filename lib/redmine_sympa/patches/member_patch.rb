      ...
      module InstanceMethods
        ...
        def register_on_sympa_mailing_list
          ...
          if(self.has_sympa_mailing_list?)
            logger.warn("[REDMINE_SYMPA] User #{self.user.name} is now member of project #{self.project.identifier}. We must register him on its mailing list.")
          end
        end

        def unregister_from_sympa_mailing_list
          if(self.has_sympa_mailing_list?)
            logger.warn("[REDMINE_SYMPA] User #{self.user.name} should be unregistered from project #{self.project.identifier}'s mailing list.")
          end
        end
      end
